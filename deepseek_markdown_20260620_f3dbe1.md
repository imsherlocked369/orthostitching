...
To prevent clusters of features in highly textured regions, the code partitions each image into a regular grid of $g_x \times g_y$ cells and retains only the top $N_{\text{per\_cell}}$ keypoints (by response) per cell. This strategy, often employed in SLAM systems (e.g., ORB‑SLAM [3]), improves spatial coverage and geometric stability of subsequent matching.
...
where $\tau = 0.75$ (Hamming distance for ORB/AKAZE binary descriptors). Mutual consistency (cross‑check) is enforced: a match is kept only if $q \rightarrow n$ and $n \rightarrow q$ are both the best mutually verified matches.
...
All image corners are projected using their global transforms. The axis‑aligned bounding box of the projected points, enlarged by a margin, defines the canvas dimensions $(W_c, H_c)$. An offset vector $\mathbf{o}$ translates the canvas origin to $(0,0)$:
...
### 7.1 Coverage Cap
A per‑pixel counter `coverage_count` tracks how many images have already contributed. New pixels are written only if the count is below a threshold $C_{\max}$ (default 3). This prevents over‑blending that can cause blurring in heavily overlapping regions.
...
## 9. Conclusion
The pipeline integrates well‑established computer vision techniques—binary features, robust pairwise estimation, multi‑scale blending, and overlap management—to produce UAV ortho‑mosaics efficiently. Its mathematical core balances accuracy and speed, making it suitable for on‑site processing.
